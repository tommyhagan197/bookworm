...
import { findBookByGutenbergId, dbPut, dbGet } from "../db/idb";
...
// ── Bible card — behaves exactly like other books ─────────────────────────────
function BibleShelfCard({ onOpenBible }) {
  const [onShelf, setOnShelf] = useState(false);
  const [adding, setAdding] = useState(false);

  useEffect(() => {
    dbGet("books", "bible-kjv").then(r => { if (r) setOnShelf(true); }).catch(() => {});
  }, []);

  async function handlePress() {
    if (onShelf) { onOpenBible && onOpenBible(); return; }
    setAdding(true);
    try {
      await dbPut("books", {
        id: "bible-kjv",
  ...
  return (
    <div className="browse-card">
      ...
      <div className="browse-card-body">
        ...
        <button
          className={"browse-btn" + (onShelf ? " in-library" : "")}
          disabled={adding}
          onClick={handlePress}
          style={{ minWidth: 120 }}
        >
          {adding ? "Adding…" : onShelf ? "✓ Open Bible" : "+ Add to Shelf"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="view-container">
      ...
      <div className="browse-search-wrap">
        ...
        />
        {query && <button className="browse-search-clear" onClick={clearSearch}>✕</button>}
      </div>

      {onOpenBible && <BibleShelfCard onOpenBible={onOpenBible} />}
      ...
      <style>{`
        .browse-search-wrap { position: relative; margin-bottom: 14px; }
        .browse-search-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); width: 16px; height: 16px; color: var(--text-muted); pointer-events: none; }
        .browse-search { width: 100%; padding: 10px 36px; background: var(--surface); border: 1px solid rgba(139,111,71,0.2); border-radius: 10px; font-size: 15px; color: var(--text); outline: none; -webkit-appearance: none; box-sizing: border-box; }
        .browse-search:focus { border-color: var(--accent); }
        .browse-search-clear { position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; font-size: 13px; color: var(--text-muted); cursor: pointer; padding: 4px; }
        .browse-loading { display: flex; flex-direction: column; align-items: center; padding: 48px 0; gap: 12px; color: var(--text-muted); font-size: 14px; }
        .browse-spinner-ring { width: 28px; height: 28px; border: 2.5px solid rgba(139,111,71,0.2); border-top-color: var(--accent); border-radius: 50%; animation: spin 0.7s linear infinite; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .browse-retry { margin-top: 8px; padding: 8px 20px; background: var(--accent); color: #fff; border: none; border-radius: 8px; font-size: 14px; cursor: pointer; }
        .browse-grid { display: flex; flex-direction: column; gap: 12px; }
        .browse-card { display: flex; gap: 14px; background: var(--surface); border-radius: 12px; padding: 12px; border: 1px solid rgba(139,111,71,0.1); align-items: flex-start; margin-bottom: 0; }
        .browse-cover { width: 64px; flex-shrink: 0; aspect-ratio: 2/3; border-radius: 5px; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 6px 4px; box-shadow: 2px 3px 8px rgba(0,0,0,0.2); gap: 4px; }
        .browse-cover-title { font-family: Georgia, serif; font-size: 8px; color: rgba(255,255,255,0.95); text-align: center; line-height: 1.3; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden; }
        .browse-cover-author { font-size: 7px; color: rgba(255,255,255,0.65); text-align: center; }
        .browse-card-body { flex: 1; display: flex; flex-direction: column; gap: 3px; min-width: 0; }
        .browse-card-title { font-family: Georgia, serif; font-size: 15px; color: var(--text); line-height: 1.3; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
        .browse-card-author { font-size: 12px; color: var(--text-muted); margin-bottom: 8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .browse-btn { align-self: flex-start; padding: 6px 14px; background: var(--accent); color: #fff; border: none; border-radius: 8px; font-size: 12px; font-weight: 500; cursor: pointer; display: flex; align-items: center; gap: 6px; min-width: 100px; justify-content: center; -webkit-tap-highlight-color: transparent; transition: opacity 0.15s; }
        .browse-btn.in-library { background: rgba(224,124,58,0.12); color: var(--accent); border: 1px solid rgba(224,124,58,0.3); }
        .browse-btn:disabled { opacity: 0.7; cursor: default; }
        .btn-spinner { width: 14px; height: 14px; animation: spin 0.7s linear infinite; }
      `}</style>
    </div>
  );
}